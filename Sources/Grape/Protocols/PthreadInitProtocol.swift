		...
		while true {
			...
			case EAGAIN,	// Not enough system resources to initialize the read-write lock
				...
				if retry < maxRetry {
					...
					let error: String = "pthread_rwlock_init failed after \(retry) retries"
					print(error)
			...
			case EINVAL:	// The attr specified in the rwlock parameter is invalid.
				let error: String = "pthread_rwlock_init failed with EINVAL: invalid attributes"
				print(error)
				return error

			default:
				// Unknown fatal error
				let error: String = "pthread_rwlock_init failed with unknown error code \(result)"
				print(error)
				return error
			}
		}
